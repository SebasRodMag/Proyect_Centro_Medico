...
    puedeVerCitas: boolean = true;
    ...
    // Se mantiene nowDate, pero su uso principal será en las funciones de TS
...
    ngOnInit(): void {
        this.nowDate = new Date(); // Asegurar que siempre esté actualizada al iniciar
...
                    return {
                        ...cita,
                        // Asegurarse de que fecha_hora_cita_combinada siempre sea una cadena válida o null
...
    }
    ...
    // Función para verificar si una cita es modificable/cancelable
    isCitaActionable(cita: any): boolean {
        // Solo se pueden modificar/cancelar citas pendientes
        if (cita.estado !== 'pendiente') {
            return false;
        }

        // Si no hay una fecha_hora_cita_combinada, la cita no es válida para esta lógica
        ...
        try {
            ...
            // Si la fecha de la cita no es una fecha válida (e.g., parseo fallido)
            if (isNaN(citaDateTime.getTime())) {
                console.warn('Fecha de cita inválida para isCitaActionable:', cita.fecha_hora_cita_combinada);
                return false;
            }

            // Compara la fecha de la cita con la fecha actual del componente
            // Se puede reiniciar nowDate si se necesita una comparación muy precisa al momento
            // this.nowDate = new Date(); // Descomentar si necesitas la hora actual exacta al momento de la verificación
...
        if (!this.isCitaActionable(cita)) { // Usar la nueva función de verificación
